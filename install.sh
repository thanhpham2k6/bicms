...
# --- 4. THIẾT LẬP MYSQL (DÙNG NHÁY ĐÔI ĐỂ NHẬN BIẾN) ---
echo "🗄️ Đang tạo Database: $DB_NAME..."

# Tạo DB và User (Bọc biến trong " " để Linux hiểu)
sudo mysql -u root -e "CREATE DATABASE IF NOT EXISTS $DB_NAME;"
sudo mysql -u root -e "CREATE USER IF NOT EXISTS '$DB_USER'@'localhost' IDENTIFIED BY '$DB_PASS';"
sudo mysql -u root -e "GRANT ALL PRIVILEGES ON $DB_NAME.* TO '$DB_USER'@'localhost';"
sudo mysql -u root -e "FLUSH PRIVILEGES;"

# Tạo bảng (Dùng dấu nháy đôi cho tên DB)
sudo mysql -u root "$DB_NAME" -e "CREATE TABLE IF NOT EXISTS users (id INT AUTO_INCREMENT PRIMARY KEY, username VARCHAR(50), password VARCHAR(255), email VARCHAR(100));"
sudo mysql -u root "$DB_NAME" -e "CREATE TABLE IF NOT EXISTS categories (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(255), slug VARCHAR(255));"
sudo mysql -u root "$DB_NAME" -e "CREATE TABLE IF NOT EXISTS posts (id INT AUTO_INCREMENT PRIMARY KEY, title VARCHAR(255), slug VARCHAR(255), content TEXT, image VARCHAR(255), user_id INT, category_id INT);"
sudo mysql -u root "$DB_NAME" -e "CREATE TABLE IF NOT EXISTS options (option_name VARCHAR(100) PRIMARY KEY, option_value TEXT);"

# --- 5. TẠO ADMIN (REPLACE ĐỂ TRÁNH LỖI DUPLICATE) ---
...
echo "=========================================="
echo "✅ XONG PHIM! WEB ĐÃ SẴN SÀNG."
